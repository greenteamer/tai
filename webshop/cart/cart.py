...
# для вывода всех вариантов доставки с ценами
# ничего не сохраняет в базу только инфо
def all_prices_for_delivery(request):
    ...
    const_list = [EMS, SPSurface, SPSAL, SPA, PS]
    ...
    i = 0
    name = {1:'EMS', 2:'SPSurface', 3:'SPSAL', 4:'SPA', 5:'PS'}

    for option_delivery in const_list:
        for key in sorted(option_delivery):
            if weight > key and weight < 20000:
                price = option_delivery[key]

        i = i+1
        sub_result = {name[i]:'%s руб.' % price}

        # result.append(sub_result)
        result.update(sub_result)

    if weight > 2000:
        result['SPSurface'] = u'Недоступно'
        result['SPSAL'] = u'Недоступно'
        result['SPA'] = u'Недоступно'
        disabled_result = {'SPSurfaceDisabled':'disabled', 'SPSALDisabled':'disabled', 'SPADisabled':'disabled',}
        result.update(disabled_result)


    if weight < 2000:
        result['PS'] = u'Недоступно'
        disabled_result = {'PSDisabled':'disabled'}
        result.update(disabled_result)
    ...
    for item in get_cart_items(request):
        if item.product.is_aqua:
            ...
            result['SPA'] = u'Недоступно'
            result['PS'] = u'Недоступно'
            disabled_result = {'SPSurfaceDisabled':'disabled', 'SPSALDisabled':'disabled', 'SPADisabled':'disabled', 'PSDisabled':'disabled'}
            result.update(disabled_result)

    return result

def status_delivery_radio(request):

    status = {'EMS':'', 'SPSurface':'', 'SPSAL':'', 'SPA':'', 'PS':''}
    weight = calculate_delivery_weight(request)
    ...
    if status['EMS'] != 'cheched':
        if weight > 2000:
            status['PS'] = 'checked'

        if weight < 2000:
            status['SPSurface'] = 'checked'
